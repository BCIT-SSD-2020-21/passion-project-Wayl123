...
export default function Home({trigger, checkUnlock}) {
  return (
    <div>
      ...
      <span className={`${styles.dot} ${trigger[0] ? styles.unlock : styles.lock}`}></span>
      <span className={`${styles.dot} ${trigger[1] ? styles.unlock : styles.lock}`}></span>
      <span className={`${styles.dot} ${trigger[2] ? styles.unlock : styles.lock}`}></span>
